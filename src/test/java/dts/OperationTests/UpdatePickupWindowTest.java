...
public class UpdatePickupWindowTest extends OperationTestUtils {
	...
	@Test
	public void testInvokeUpdatePickupWindow() {
		...
		PickupTime timeBeforeUpdate = getPickupTimeForTheNextHoursFromNow(1, 3);
		PickupTime timeAfterUpdate = getPickupTimeForTheNextHoursFromNow(3, 5);
		PickupLocation locationAfterUpdate = new PickupLocation(2.5, 2.5);
		List<Food> foodAfterUpdate = new ArrayList<Food>();
		foodAfterUpdate.add(new Food("Marshmallow", 5));
		foodAfterUpdate.add(new Food("Chocolate", 3)); // before the update it was marshmallow == 3, chocolate == 4

		invokeCreatePickupWindowOperationOnItem(ob, OperationTestConstants.DONATOR_NAME, timeBeforeUpdate,
		...
		assertPickupWindowOnItem(createdItem, OperationTestConstants.DONATOR_NAME, timeBeforeUpdate,
				OperationTestConstants.PICKUP_LOCATION, OperationTestConstants.FOOD_LIST,
				OperationTestConstants.RECEIVERS_LIST);

		invokeUpdatePickupWindowOperationOnItem(ob, OperationTestConstants.DONATOR_NAME, timeAfterUpdate,
				locationAfterUpdate, foodAfterUpdate, OperationTestConstants.RECEIVERS_LIST);

		createdItem = getItem(getUserSpace(userWithPlayerRole), getUserEmail(userWithPlayerRole),
				currentItem.getItemId().getSpace(), currentItem.getItemId().getId());

		assertPickupWindowOnItem(createdItem, OperationTestConstants.DONATOR_NAME, timeAfterUpdate, locationAfterUpdate,
				foodAfterUpdate, OperationTestConstants.RECEIVERS_LIST);
	}

	private void invokeUpdatePickupWindowOperationOnItem(OperationBoundary ob, String donatorName,
			PickupTime pickupTime, PickupLocation pickupLocation, List<Food> listOfFood, List<Reciever> receiversList) {
		ob.setType(OperationTestConstants.UPDATE_PICKUP_WINDOW);

		OperationBoundary updatePickupWindowOp = packArgsToAnOperationBoundary(ob, donatorName, pickupTime,
				pickupLocation, listOfFood, receiversList);

		this.operationTest.invokeOperationOnItem(url, updatePickupWindowOp);
	}

	private void invokeCreatePickupWindowOperationOnItem(OperationBoundary ob, String donatorName,
			PickupTime pickupTimeFromNowToProvidedHours, PickupLocation pickupLocation, List<Food> foodList,
			List<Reciever> receiversList) {

		ob.setType(OperationTestConstants.CREATE_PICKUP_WINDOW);

		OperationBoundary createPickupWindowOp = packArgsToAnOperationBoundary(ob, OperationTestConstants.DONATOR_NAME,
